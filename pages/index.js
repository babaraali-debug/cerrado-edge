...
const SCAN_UNIVERSE = [
  ...
  'ORCL','INTC','QCOM','AVGO','MU','PLTR','SNOW','UBER','LYFT','ABNB',
  'JPM','BAC','GS','MS','WFC','V','MA','PYPL','SQ',
  'XOM','CVX','BA','CAT','GE','F','GM',
  'JNJ','PFE','MRNA','ABBV','UNH',
  'WMT','TGT','COST','NKE','MCD','SBUX','DIS',
  'GME','AMC','RIVN','SOFI','LCID','HOOD',
  'CNQ.TO','SU.TO','RY.TO','TD.TO','BNS.TO','SHOP.TO','ABX.TO','CNR.TO',
  'LUCA.V','IFOS.V',
];
...
function gradeOrder(grade) {
  return { 'A': 0, 'B': 1, 'C': 2, 'D': 3, 'F': 4 }[grade] ?? 5;
}
          ...
          return (
            <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
              ...
              <div style={{ width: '100%', background: '#f0f4ff', borderRadius: 3, height: 40, display: 'flex', alignItems: 'flex-end' }}>
                <div style={{ width: '100%', height: Math.max(4, pct * 0.4) + 'px', background: col, borderRadius: 3, opacity: 0.8 }} />
              </div>
              <div style={{ fontSize: 9, color: '#a0aec0' }}>D{d.day}</div>
            </div>
          );
      ...
      {valid.length > 0 && (() => {
        ...
        return (
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <div style={{ fontSize: 10, color: '#16a34a', background: '#f0fdf4', border: '1px solid #bbf7d0', padding: '3px 8px', borderRadius: 6, fontWeight: 600 }}>★ Best exit: Day {best.day} ({best.winRate}% win rate)</div>
            <div style={{ fontSize: 10, color: '#dc2626', background: '#fef2f2', border: '1px solid #fecaca', padding: '3px 8px', borderRadius: 6 }}>Weakest: Day {worst.day} ({worst.winRate}% win rate)</div>
          </div>
        );
      })()}
    </div>
...
export default function Home() {
  ...
  const runScan = async () => {
    ...
    for (let i = 0; i < SCAN_UNIVERSE.length; i++) {
      const t = SCAN_UNIVERSE[i];
      setScanStatus(`Scanning ${t}...`);
      setScanProgress(Math.round((i / SCAN_UNIVERSE.length) * 100));
      try {
        ...
        if (activeSignal) {
          ...
          hits.push({ ticker: t, price: stats.closes[0].toFixed(2), change: stats.changes[0].toFixed(2), activeSignal, patterns, randomBaseline, stats, confidence, plainDesc: PATTERN_PLAIN[activeSignal.name] || activeSignal.desc });
        }
      } catch (e) { /* skip */ }
    ...
    <>
      ...
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .scan-row { cursor: pointer; transition: all 0.2s; }
        .scan-row:hover { background: #f8f9ff; }
      `}</style>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '32px 24px 80px' }}>
        ...
        <div style={{ textAlign: 'center', padding: '40px 20px 32px', maxWidth: 580, margin: '0 auto 32px' }}>
          ...
          <h1 style={{ fontFamily: 'Syne,sans-serif', fontSize: 40, fontWeight: 800, lineHeight: 1.1, marginBottom: 12, color: N }}>
            {mode === 'scanner' ? <>Morning<br />Scanner</> : <>Movement<br />Analyzer</>}
          </h1>
          <p style={{ color: MUTED, fontSize: 13, lineHeight: 1.8 }}>
            {mode === 'scanner' ? `Scans ${SCAN_UNIVERSE.length} stocks · Finds today's patterns · Ranked by confidence` : 'Real price history · 6 patterns · Random baseline · Kelly sizing · Edge decay curves'}
          </p>
        </div>
        ...
        {mode === 'scanner' && (
          <div className="fade">
            <div className="card" style={{ marginBottom: 24, padding: '28px', textAlign: 'center' }}>
              <div style={{ fontSize: 13, color: MUTED, marginBottom: 6, lineHeight: 1.8 }}>
                No ticker needed. Hit the button and we scan <strong style={{ color: TEXT }}>{SCAN_UNIVERSE.length} stocks</strong> for you.<br />
                Only shows stocks where a real pattern triggered <strong style={{ color: TEXT }}>today</strong>.
              </div>
              <div style={{ fontSize: 11, color: '#a0aec0', marginBottom: 22 }}>Takes about 90 seconds · Results ranked A → F by confidence</div>
              ...
              {scanning && (
                <div style={{ marginTop: 20 }}>
                  ...
                  <div style={{ fontSize: 11, color: MUTED, animation: 'scanpulse 1.5s ease infinite' }}>{scanStatus} — {scanProgress}% complete</div>
                </div>
              )}
            </div>
            ...
            {!scanning && scanProgress === 100 && scanResults.length === 0 && (
              <div className="card" style={{ textAlign: 'center', padding: 40 }}>
                <div style={{ fontSize: 28, opacity: 0.1, marginBottom: 10 }}>◎</div>
                <div style={{ color: MUTED, fontSize: 13, fontWeight: 500 }}>No active signals found today</div>
                <div style={{ color: '#cbd5e0', fontSize: 11, marginTop: 5 }}>The market is quiet — no patterns forming right now</div>
              </div>
            )}
            ...
            {scanResults.length > 0 && (
              <div className="fade">
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                  <div style={{ fontSize: 11, color: MUTED, letterSpacing: 2, textTransform: 'uppercase', fontWeight: 500 }}>{scanResults.length} Signal{scanResults.length !== 1 ? 's' : ''} Found Today</div>
                  <div style={{ fontSize: 10, color: '#a0aec0' }}>{SCAN_UNIVERSE.length} stocks scanned</div>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {scanResults.map((r, i) => {
                    ...
                    return (
                      <div key={i} className="card" style={{ padding: '18px 20px', cursor: 'pointer', transition: 'box-shadow 0.2s' }}
                        onClick={() => setExpandedScan(isExp ? null : i)}>
                        ...
                        <div style={{ marginTop: 12, fontSize: 12, color: TEXT, background: BG, borderRadius: 8, padding: '9px 14px', lineHeight: 1.6 }}>
                          {r.plainDesc}
                        </div>
                        ...
                        {isExp && (
                          <div style={{ marginTop: 18, borderTop: `1px solid ${BORDER}`, paddingTop: 18 }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 14 }}>
                              {[
                                { label: 'Entry',     val: '$' + r.activeSignal.entry,    col: TEXT,      sub: 'Current price' },
                                { label: 'Stop Loss', val: '$' + r.activeSignal.stopLoss, col: '#dc2626', sub: 'Max loss: ' + r.activeSignal.maxAdverse },
                                { label: 'Target',    val: '$' + r.activeSignal.target,   col: '#16a34a', sub: 'R/R: ' + r.activeSignal.rr },
                              ].map(tc => (
                            ...
                            {r.activeSignal.decay && (
                              <div style={{ background: BG, borderRadius: 10, padding: '14px 16px', marginBottom: 14 }}>
                                <DecayCurve decay={r.activeSignal.decay} randomBaseline={r.randomBaseline} />
                              </div>
                            )}
              ...
              <div className="fade">
                ...
                {sig ? (
                  <div style={{ background: sigBg, border: `2px solid ${sigCol}33`, borderRadius: 14, padding: 22, marginBottom: 24 }}>
                    ...
                    {(() => {
                      const c = getConfidence(sig.winRate, result.randomBaseline, sig.instances, sig.kelly, sig.decay);
                      return <ConfidenceGauge score={c.score} grade={c.grade} label={c.label} color={c.color} components={c.components} patternName={sig.name} />;
                    })()}
                  ...
                  {result.patterns.map((p, i) => {
                    ...
                    return (
                      <div key={i} className="card pat-card" style={{ padding: '14px 16px', opacity: p.reliable ? 1 : 0.7 }}
                        ...
                        {(() => {
                          const c = getConfidence(p.winRate, result.randomBaseline, p.instances, p.kelly, p.decay);
                          return <ConfidenceGauge score={c.score} grade={c.grade} label={c.label} color={c.color} components={c.components} patternName={p.name} />;
                        })()}
                        ...
                        {isExpanded && (
                          <div style={{ borderTop: `1px solid ${BORDER}`, paddingTop: 12, marginTop: 4 }}>
                            <DecayCurve decay={p.decay} randomBaseline={result.randomBaseline} />
                          </div>
                        )}
                      </div>
                    );
