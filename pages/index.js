...
function DecayCurve({decay,randomBaseline}){
  ...
  return(
    <div style={{marginTop:10}}>
      <div style={{fontSize:10,color:'#6b7ab5',letterSpacing:1,marginBottom:8,fontWeight:500}}>SIGNAL STRENGTH BY DAY</div>
      <div style={{display:'flex',gap:6,alignItems:'flex-end',height:60,marginBottom:6}}>
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:3}}><div style={{fontSize:9,color:'#94a3b8'}}>{randomBaseline}%</div><div style={{width:'100%',background:'#f0f4ff',borderRadius:3,height:40,display:'flex',alignItems:'flex-end'}}><div style={{width:'100%',height:Math.max(4,((randomBaseline-minWR)/range)*40)+'px',background:'#94a3b8',borderRadius:3,opacity:0.5}}/></div><div style={{fontSize:9,color:'#a0aec0'}}>RAND</div></div>
      </div>
      {valid.length>0&&(()=>{const best=valid.reduce((b,d)=>d.winRate>b.winRate?d:b);const worst=valid.reduce((b,d)=>d.winRate<b.winRate?d:b);return(<div style={{display:'flex',gap:8,flexWrap:'wrap'}}><div style={{fontSize:10,color:'#16a34a',background:'#f0fdf4',border:'1px solid #bbf7d0',padding:'3px 8px',borderRadius:6,fontWeight:600}}>★ Best exit: Day {best.day} ({best.winRate}% win rate)</div><div style={{fontSize:10,color:'#dc2626',background:'#fef2f2',border:'1px solid #fecaca',padding:'3px 8px',borderRadius:6}}>Weakest: Day {worst.day} ({worst.winRate}% win rate)</div></div>);})()}
    </div>
  );
}
...
function HeroSignal({r,rank,onDive}){
  ...
  return(
    <div style={{background:'linear-gradient(135deg, #0f1117 0%, #161b27 100%)',border:`1px solid ${col}33`,borderRadius:16,padding:'20px 22px',boxShadow:`0 0 32px ${col}15`,flex:1,minWidth:240}}>
          ...
          <div className="fade">
            <div className="card" style={{marginBottom:20,padding:'24px 28px',textAlign:'center'}}>
              ...
              <div style={{display:'flex',gap:10,justifyContent:'center',alignItems:'center',marginBottom:20,flexWrap:'wrap'}}>
                ...
                <input value={portfolio} onChange={e=>setPortfolio(e.target.value)} placeholder="e.g. 50000"
                  style={{width:140,background:BG,border:`1.5px solid ${BORDER}`,borderRadius:8,padding:'8px 12px',color:TEXT,fontSize:13,outline:'none',textAlign:'center'}}
            ...
            {filteredResults.length>0&&(
              <div className="fade">
                <div style={{fontSize:11,color:MUTED,letterSpacing:2,textTransform:'uppercase',fontWeight:500,marginBottom:12}}>All Signals</div>
                          ...
                          <div style={{display:'flex',alignItems:'center',gap:12}}>
                            ...
                            <div>
                              ...
                              <div style={{fontSize:11,color:MUTED,marginTop:3}}>
                                ...
                                &nbsp;·&nbsp;<strong style={{color:'#d97706'}}>{signalExpiry(r.activeSignal.bestDay)}</strong>
                              </div>
                            </div>
                          </div>
                          <div style={{display:'flex',alignItems:'center',gap:10}}>
                        ...
                        {isExp&&(
                          <div style={{marginTop:16,borderTop:`1px solid ${BORDER}`,paddingTop:16}}>
                            ...
                            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10,marginBottom:14}}>
                              {[{label:'Entry',val:'$'+r.activeSignal.entry,col:TEXT,sub:'Current price'},{label:'Stop Loss',val:'$'+r.activeSignal.stopLoss,col:'#dc2626',sub:'Max loss: '+r.activeSignal.maxAdverse},{label:'Target',val:'$'+r.activeSignal.target,col:'#16a34a',sub:'R/R: '+r.activeSignal.rr}].map(tc=>(<div key={tc.label} style={{background:BG,borderRadius:10,padding:'12px 14px',textAlign:'center'}}><div style={{fontSize:9,color:MUTED,letterSpacing:1,marginBottom:5,textTransform:'uppercase',fontWeight:500}}>{tc.label}</div><div style={{fontFamily:'Syne,sans-serif',fontSize:17,fontWeight:800,color:tc.col}}>{tc.val}</div><div style={{fontSize:9,color:MUTED,marginTop:3}}>{tc.sub}</div></div>))}
                            </div>
            ...
            <div className="card" style={{marginBottom:24,padding:'24px 28px'}}>
              <div style={{display:'flex',gap:10,marginBottom:14,flexWrap:'wrap'}}>
                <input value={ticker} onChange={e=>setTicker(e.target.value.toUpperCase())} onKeyDown={e=>e.key==='Enter'&&run()}
                  placeholder="TICKER  (e.g. MSFT · AAPL · CNQ.TO)"
            ...
            {!loading&&result&&s&&(
              <div className="fade">
                ...
                <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(170px,1fr))',gap:10,marginBottom:24}}>
                  {[{label:'Avg daily move',val:s.avgAbsChange.toFixed(2)+'%',sub:'Directional: '+(s.avgChange>=0?'+':'')+s.avgChange.toFixed(2)+'%',col:'#16a34a'},{label:'Biggest single day gain',val:'+'+s.maxGain.toFixed(2)+'%',sub:'Historical max',col:'#16a34a'},{label:'Biggest single day drop',val:s.maxLoss.toFixed(2)+'%',sub:'Historical max',col:'#dc2626'},{label:'Daily volatility',val:s.stdDev.toFixed(2)+'%',sub:'Annual: '+s.annualVol.toFixed(1)+'%',col:N},{label:'Avg intraday range',val:s.avgIntraday.toFixed(2)+'%',sub:'Max: '+s.maxIntraday.toFixed(2)+'%',col:N},{label:'Avg daily volume',val:fmtVol(s.avgVol),sub:'Last: '+fmtVol(s.volumes[0]),col:N}].map((c,i)=>(<div key={i} className="card" style={{padding:'14px 16px'}}><div style={{fontSize:10,color:MUTED,marginBottom:6,fontWeight:500}}>{c.label}</div><div style={{fontFamily:'Syne,sans-serif',fontSize:22,fontWeight:800,color:c.col}}>{c.val}</div><div style={{fontSize:10,color:MUTED,marginTop:3}}>{c.sub}</div></div>))}
                </div>
                    ...
                    return(
                      <div key={i} className="card pat-card" style={{padding:'14px 16px',opacity:p.reliable?1:0.7}} onClick={()=>setExpandedPattern(isExpanded?null:i)}>
                        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:6,flexWrap:'wrap',gap:6}}>
                          <div style={{display:'flex',alignItems:'center',gap:8}}><span style={{fontSize:13,color:TEXT,fontWeight:600}}>{p.name}</span>{!p.reliable&&<span style={{fontSize:9,color:'#d97706',background:'#fffbeb',border:'1px solid #f5c84266',padding:'2px 7px',borderRadius:10,fontWeight:600}}>⚠️ LOW SAMPLE ({p.instances})</span>}</div>
